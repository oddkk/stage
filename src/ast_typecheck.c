...
static int
ast_node_resolve_types_old(struct ast_context *ctx, struct ast_module *mod,
		struct ast_env *env, struct ast_node *node)
{
	int errors = 0;
#define VISIT_NODE(child) errors += ast_node_resolve_types_old(ctx, mod, env, child);
...
#undef VISIT_NODE
	...
	switch (res->result) {
		case AST_SLOT_RESULT_ERROR:
			// The error should already have been reported.
			errors += 1;
			break;

		case AST_SLOT_RESULT_UNKNOWN:
			stg_error(ctx->err, node->loc,
		...
		case AST_SLOT_RESULT_FOUND_VALUE_OBJ:
		case AST_SLOT_RESULT_FOUND_VALUE_TYPE:
		case AST_SLOT_RESULT_FOUND_TYPE:
			node->type = res->type;
			break;
	}
		...
		type_id expected_type)
{
	...
	err = ast_node_resolve_types_old(ctx, mod, env, node);
